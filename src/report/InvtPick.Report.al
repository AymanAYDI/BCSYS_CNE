report 50047 "Invt. Pick"
{
    ...
    // 
    // //>>CNE5.00
    // TDL.27:20131120/CSC Design
    // YourRef -> 30 old -> 35 new
    // 
    // //>>MODIF HL
    // TI251206 DO.GEPO 19/11/2014 : add C/AL code on
    // 
    // //>>MODIF HL
    // TI269037 DO.GEPO 10/03/2015 : modify WhseActLine - OnAfterGetRecord()
    // 
    // //>>MODIF HL
    // TI270080 DO.GEPO 16/03/2015 : comment modif TI269037
    // -----------------------------------------------
    DefaultLayout = RDLC;
        ...
        dataitem(WhseActivityHeader; Table5766)
        {
            ...
            dataitem(Copy; Table2000000026)
            {
                ...
                dataitem(PageLoop; Table2000000026)
                {
                    ...
                    column(WhseActivityHeader__Sales_Counter_; FORMAT(WhseActivityHeader."Sales Counter"))
                    {
                    }
                    column(WhseActivityHeader__Bin_Code_; WhseActivityHeader."Bin Code")
                    {
                    }
                    column(WhseActivityHeader__Destination_Name_; WhseActivityHeader."Destination Name")
                    {
                    }
                    column(WhseActivityHeader_Comments; WhseActivityHeader.Comments)
                    {
                    }
                    ...
                    column(WhseActivityHeader__Sales_Counter_Caption; WhseActivityHeader.FIELDCAPTION("Sales Counter"))
                    {
                    }
                    column(WhseActivityHeader__Bin_Code_Caption; WhseActivityHeader.FIELDCAPTION("Bin Code"))
                    {
                    }
                    ...
                    dataitem(WhseActLine; Table5767)
                    {
                        ...
                        column(BooGBinItemVisible;BooGBinItemVisible)
                        {
                        }
                        column(BinDetailed;BinDetailed)
                        {
                        }
                        column(ShowOutStock;ShowOutStock)
                        {
                        }
                        dataitem(EmptyLine;Table2000000026)
                        {
                            DataItemTableView = SORTING(Number)
                                                ORDER(Ascending);
                        ...
                        begin
                            ...
                            //>>TI251206
                            //IF ("Bin Code" = Location."Receipt Bin Code") THEN
                            //  BEGIN
                            //<<TI251206
                                CLEAR(BarcodeMngt);
                                EAN13Bar := '';
                                ...
                                EAN13BarTxt := '';
                                CLEAR(DistInt);
                                EAN13Bar := COPYSTR(DistInt.GetItemEAN13Code("Item No."),1,MAXSTRLEN(EAN13Bar));
                                IF (EAN13Bar <> '') THEN
                            ...
                            "Source Document"::"Sales Order":
                              BEGIN
                                ...
                                IF NOT TmpSalesLine.GET(SalesLine."Document Type",SalesLine."Document No.",SalesLine."Line No.") THEN
                                  BEGIN
                                    TmpSalesLine.INIT;
                                ...
                                END;
                                //>>TI269037 prise en compte du meme article plusieurs fois dans la cmd
                                //>>TI270080
                                /*SalesLine.RESET;
                                SalesLine.SETRANGE("Document Type",WhseActLine."Source Subtype");
                                SalesLine.SETRANGE("Document No.",WhseActLine."Source No.");
                                SalesLine.SETFILTER("Line No.",'<>%1',WhseActLine."Source Line No.");
                                SalesLine.SETRANGE("No.",WhseActLine."Item No.");
                    ...
                    dataitem(OutStockLoop;Table2000000026)
                    {
                        DataItemTableView = SORTING(Number)
                                            ORDER(Ascending);
                        ...
                        begin
                            ...
                            CLEAR(AvailableQty);
                            CLEAR(Item);
                            IF TmpSalesLine2.Type = TmpSalesLine2.Type::Item THEN
                              BEGIN
            ...
            begin
                ...
                WhseActivityHeader."Source Document"::"Sales Order":
                  BEGIN
                    IF NOT WhseActivityHeader."Sales Counter" THEN
                    ...
                    IF NOT ShipmentMethod."To Make Available" THEN
                      FormatAddr.SalesHeaderShipTo(ShipToAddr,BillToAddr,SalesHeader);
    ...
    requestpage
    {
        ...
        begin
            ...
            //TECSO 15/12/03 CST : Raz du Nbre de Copies forcé
