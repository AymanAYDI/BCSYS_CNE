codeunit 50015 "DocMatrix Management"
{
    ...
    // DEL/PD/20220215/CRQ002 : changed functions: "Sell-to" to "Bill-to"
    //                                - f.GetPostedSalesInvoice
    //                                - f.FillMailPlaceHolderArray
    //                                - f.GetPostedSalesCreditMemo
    //                                - f.GetEmailFromSalesOrder
    // DEL/PD/20220609/CRQ002 : new f.PostSaleOrderAndDocMatrixPrint
    //                          changed f.ShowDocMatrixSelection: new parameter pShowPage
    // DEL/PD/20221005/CRQ002 : merged change to PROD
    ...
    [Scope('Internal')]
    procedure PostSaleOrderAndDocMatrixPrint(pSalesHeader: Record "36"; pShowDocMatrixSelectionPage: Boolean): Boolean
    var
        ...
        precDealShipmentSelection: Record "50031";
        pcdUpdateRequestID: Code[20];
        pboShipmentSelected: Boolean;
    begin
        // note: here the function is called with "lUsage::S.Order" and "lboSalesOrderConfirmation = FALSE"
        lboSalesOrderConfirmation := FALSE;
        IF ShowDocMatrixSelection(pSalesHeader."Bill-to Customer No.", ProcessType::Manual, lUsage::"S.Order", lrecDocMatrixSelection, lboSalesOrderConfirmation, pShowDocMatrixSelectionPage) THEN BEGIN
            ...
            IF lrecDocMatrixSelection.Post <> lrecDocMatrixSelection.Post::" " THEN BEGIN
                ...
                IF ApplicationAreaSetup.IsFoundationEnabled THEN
                    LinesInstructionMgt.SalesCheckAllLinesHaveQuantityAssigned(pSalesHeader);
                IF NOT pSalesHeader.IsApprovedForPosting() THEN
                    EXIT(FALSE);

                // post the Sales Order acording user DocMatrixSelection
                pSalesHeader.Invoice := lrecDocMatrixSelection.Post IN [2, 3];
                pSalesHeader.Ship := lrecDocMatrixSelection.Post IN [1, 3];
                CODEUNIT.RUN(CODEUNIT::"Sales-Post", pSalesHeader);
                ...
                IF GetPostedSalesInvoice(pSalesHeader."No.", pSalesHeader."Bill-to Customer No.", lrecSalesInvoiceHeader) THEN BEGIN
                    ProcessDocumentMatrix(lUsage::"S.Invoice", ProcessType::Manual, lrecSalesInvoiceHeader,
                                          lrecSalesInvoiceHeader.FIELDNO("Bill-to Customer No."),
                                          lrecSalesInvoiceHeader.FIELDNO("No."), lrecDocMatrixSelection, 0);
                    EXIT(TRUE);
                END;
    ...
    [Scope('Internal')]
    procedure ShowDocMatrixSelection(pNo: Code[20]; pProcessType: Option Manual,Automatic; pUsage: Integer; var precDocMatrixSelection: Record "50071"; pPrintOnly: Boolean; pShowPage: Boolean): Boolean
    var
        ...
        CASE pUsage OF
            ...
            1:
                BEGIN
                    ...
                    //'Bill-to Customer No.'
                    FieldRef := RecRef.FIELD(lrecSalesHeader.FIELDNO("Bill-to Customer No."));
    ...
    begin
        ...
        precSalesInvoiceHeader.SETRANGE("Bill-to Customer No.", pCustNo);
    ...
    begin
        ...
        precSalesCrMemoHeader.SETRANGE("Bill-to Customer No.", pCustNo);
    ...
    begin
        IF lrecSalesOrder.GET(lrecSalesOrder."Document Type"::Order, pDocNo) THEN
            IF lrecContact.GET(lrecSalesOrder."Bill-to Contact No.") THEN BEGIN
                IF lrecContact."E-Mail" = '' THEN BEGIN
                    IF GUIALLOWED THEN
                        ERROR(Err002, lrecSalesOrder."Bill-to Contact No.", pDocNo)
                END ELSE
